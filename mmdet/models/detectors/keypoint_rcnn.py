...
class KeypointRCNN(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
        ...
        if keypoint_head is not None:
            ...
            if not isinstance(keypoint_head, list):
                keypoint_head = [keypoint_head for _ in range(num_stages)]
            assert len(keypoint_head) == self.num_stages
            for head in keypoint_head:
                self.keypoint_head.append(builder.build_head(head))
            if keypoint_roi_extractor is not None:
                self.share_roi_extractor = False
                self.keypoint_roi_extractor = nn.ModuleList()
                if not isinstance(keypoint_roi_extractor, list):
                    keypoint_roi_extractor = [
                        keypoint_roi_extractor for _ in range(num_stages)
                    ]
    ...
    def init_weights(self, pretrained=None):
        super(KeypointRCNN, self).init_weights(pretrained)
        ...
        for i in range(self.num_stages):
            ...
            if self.with_keypoint:
                if not self.share_roi_extractor:
                    self.keypoint_roi_extractor[i].init_weights()
                self.keypoint_head[i].init_weights()
